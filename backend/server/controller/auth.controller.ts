...
export const Login = async (req: Request, res: Response) => {
  try {
    ...
    if (!isEmailVailed) {
      logger.log("auth", {
        ip: req.ip,
        message: `[validator]: Failed email check`,
        type: "error",
        route: "POST /login",
      });
    ...
    const [exists, fields]: any = await pool.query(
      "SELECT id, name, profile_image, password FROM users WHERE email = ?",
      [email],
    );
    ...
    if (exists.length != 0) {
      ...
      logger.log("auth", {
        ip: req.ip,
        message: `Login attempt from invailed email address ${email}`,
        type: "error",
        route: "POST /login",
      });
      send.unauthorized(res, "Invailed Credentials");
...
export const Register = async (req: Request, res: Response) => {
  try {
    ...
    if (!isUsernameVailed) {
      logger.log("auth", {
        ip: req.ip,
        message: `[validator]: Failed username check`,
        type: "error",
        route: "POST /register",
      });
    ...
    if (!isPasswordVailed) {
      logger.log("auth", {
        ip: req.ip,
        message: `[validator]: Failed password check`,
        type: "error",
        route: "POST /register",
      });
      return send.badRequest(
    ...
    const saltRounds: number =
      (typeof process.env.SALTROUNDS == "string" &&
    ...
    if (exists.length != 0) {
      ...
      logger.log("auth", {
        ip: req.ip,
        message: `User Created successfully with email address ${email}`,
        type: "success",
        route: "POST /register",
      });
