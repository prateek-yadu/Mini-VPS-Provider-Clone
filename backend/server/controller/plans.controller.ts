...
export const Plans = async (req: Request, res: Response) => {
  try {
    ...
    const [plans, fields]: any = await pool.query(
      "SELECT id, name, description, price, vCPU, memory, storage, backups, validity_days FROM plans",
    );
...
export const purchasePlan = async (req: customRequest, res: Response) => {
  try {
    ...
    const [plan, fields]: any = await pool.query(
      "SELECT name, validity_days FROM plans WHERE id = ?",
      [planId],
    ); // gets requested plan
    ...
    if (plan.length != 0) {
      ...
      const [userPlan, fields]: any = await pool.query(
        "INSERT INTO user_plans (id, user_id, plan_id, expires_at) VALUES (?, ?, ?, (SELECT DATE_ADD((SELECT NOW()), INTERVAL ? DAY)))",
        [userPlanTableId, userId, planId, planValidity],
      );
      ...
      logger.log("billing", {
        ip: req.ip,
        message: `Trying to Purchase non-existing plan`,
        type: "error",
        route: "POST /:id/purchase",
        userId: req.id,
      });
